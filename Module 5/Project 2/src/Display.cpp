...
void Display::displayInputScreen(Account& t_account) {
    ...
    while (!(cin >> monthlyDeposit) ||
           monthlyDeposit < 0) {  // loop while input is not a number or is negative
        cout << "Invalid input. Please enter a positive number: $";
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Clear the input buffer
    }
    ...
    while (!(cin >> annualInterest) ||
           annualInterest < 0) {  // loop while input is not a number or is negative
        cout << "Invalid input. Please enter a positive number: %";
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Clear the input buffer
    }
    ...
    while (!(cin >> numberOfYears) ||
           numberOfYears <= 0) {  // loop while input is not a number or is negative
...
void Display::printDetails(int t_year, double t_yearEndBalance, double t_interestEarned) {
    cout << right << setw(6) << t_year << fixed << setprecision(2) << setw(24) << "$"
         << t_yearEndBalance << setw(28) << "$" << t_interestEarned << endl;
